# Program to solve a 2-D Heat Flow
    ...
    for i in range(size_list):
        for j in range(size_list):
            ...
            if x == 0 or y == 0 or y == side_size - 1:
                ...
                diff_temp[i, j] = r * (global_temp[x, y + 1] + global_temp[
                                       x, y - 1] + 2 * global_temp[x - 1, y] - 4 * global_temp[x, y]) / 4
            elif x >= 0 and y >= 0 and x < side_size and y < side_size:
                diff_temp[i, j] = r * (global_temp[x + 1, y] + global_temp[x, y + 1] + global_temp[
                                       x, y - 1] + global_temp[x - 1, y] - 4 * global_temp[x, y]) / 4
            else:
                print(
                    "Index out of bounds - x - {} y - {} i - {} j - {} rank - {}").format(x, y, i, j, rank)
