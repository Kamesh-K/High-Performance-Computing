...
# Scattering the array to different processes to do computations in those processes 
...
time_step = 0 
# Declaring elements which are received from other processes for computing the temperature evolution
left_elem = None 
right_elem = None 
...
while time_step < max_time_step:
	# All the itermediate values in a sub domain is calculated for which no data is required to be exchanged from the nearby processes
	for i in range(1,block_size-1):
		temp_2[i]= temp_1[i] + r*(temp_1[i+1] - 2*temp_1[i] + temp_1[i-1])
	# For rank = 0, we need to receive only one data from rank = 1 for the final element in rank 0 
	if rank == 0:
	...
	# For rank = size - 1, we need to receive only one data from rank = size - 2 for the final element in rank size - 1 
	elif rank == size - 1:
